...
#define NAU7802_I2CADDR_DEFAULT 0x2A //Default I2C address for the NAU7802

#define DELAY_1MS vTaskDelay(1 / portTICK_PERIOD_MS)
#define CURRENT_TIME_MS (esp_timer_get_time()/1000) //Get current time in milliseconds 
...
public:
  ...
  bool waitForCalibrateAFE(int64_t timeout_ms = 0); //Wait for asynchronous AFE calibration to complete with optional timeout.
